...
- 若 Radeon 驱动后续仍在 SeedVC 主图中抛出未覆盖的 DirectML 算子异常，worker 会记录原始 traceback 并自动以完整 CPU 稳定路径重试；CPU/CUDA/ROCm 的正常错误不会被重复执行。
...
- 更正用户数据目录名称：新安装统一使用 `.xb_svcb`；已有默认 `.sb-svcb` 会在启动时安全同盘重命名并更新数据指针，失败时自动回滚继续使用旧目录，模型、作品与设置不会丢失。
