...
产物：dist/XB-SVCB/XB-SVCB.exe（+ _internal/）。其中：
  ...
  - 三个 worker 脚本（svc/f0/uvr）以「真实磁盘文件」形式放进 _internal/infrastructure，
    供外部 .venv-svc / .venv-uvr 的 Python 以子进程读取执行；
...
for w in ("svc_worker.py", "f0_worker.py", "uvr_worker.py"):
