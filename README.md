...
- 🎧 **作品库** —— 试听 / 导出成品，单独试听伴奏与干声，失败任务一键查日志。

> **最新版本 v0.0.2**：新增双主题切换、原生窗口边框主题化、自定义头像与消息通知、专属应用图标。
...
3. **自动处理** —— 分离 → 去混响 → F0 提取 → SVC 推理 → ffmpeg 合并伴奏。
4. **作品库** —— 试听 / 导出成品，单独试听**伴奏**与**干声**；失败任务一键打开日志。
