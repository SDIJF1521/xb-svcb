...
- **效果器与插件**：片段可叠加混响、降噪、噪声门、压缩、EQ、高通、低通、延迟、合唱、限幅、增益等内置效果器；外部 VST3 效果器走 `Python -> C++ JUCE VST3 Host -> VST3 Plugin GUI`，前端提供组件化「插件窗口」弹窗，JUCE Host 负责 VST3 检查/加载、离线渲染、原生插件 GUI 与插件 state 回写。混音播放中修改效果或其他声音参数会自动在后台重渲染，完成后保持当前播放位置热切换，无需手动暂停再播放。
