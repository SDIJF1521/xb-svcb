...
3. 在「选择用户数据存储位置」页选择 `.xb_xvcb` 数据目录（默认 `{安装目录}\.xb_xvcb`）。模型、作品、下载素材、编辑工程与缓存都会保存在这里，C 盘空间不足时建议选 D/E 盘。
...
- 默认用户数据目录为 `.xb_xvcb`，用于保存模型库、作品、在线下载素材、音频编辑工程、波形/渲染缓存和配置文件。
- 安装时可在「选择用户数据存储位置」页把 `.xb_xvcb` 放到空间充足的磁盘，避免占满 C 盘。
...
- 旧版本目录 `.xb-svcb` / `.xb_svcb` 仍会被兼容识别，升级时不会丢失已有数据。
...
- 卸载时清理安装目录内生成的 `.venv-*`、`engines/`、`models/`；用户作品数据位于 `.xb_xvcb`（可在安装时或软件首页选择/迁移），予以保留。
