...
- `public_ip` 与 `public_domain` **只用于生成外部访问地址**，不会被传给 Uvicorn 作为监听地址，因此不会错误地尝试绑定 `198.51.100.10`。
- 文档和测试使用保留示例值 `198.51.100.10`、`api.example.test`；运行时不会固定任何公网 IP、域名或端口。
- 端口仍由 API 页面配置，`8765` 只是默认值；域名配置后文档和调用地址优先使用域名。

## 域名访问行为

- `docs_url`、`redoc_url` 和前端文档调用示例会根据当前服务范围与配置动态生成。
- 局域网模式配置域名后，用户可使用 `http://配置的域名:配置的端口/docs` 访问文档并调用 `/api/v1/...`；本机模式仍显示 `127.0.0.1`，避免把不可达域名误认为监听地址。
- DNS 解析、路由器/云服务器端口转发、安全组和 Windows 防火墙仍需由部署环境配置，程序不会擅自修改这些系统设置。
...
26 passed, 1 warning, 5 subtests passed
```
...
代码级测试已验证 `198.51.100.10` 和 `api.example.test` 会被保存、规范化并生成访问地址，同时监听地址仍为 `0.0.0.0`。由于本机公网 IP 无法被外部访问，公网连通性不能仅靠本机代码判定；还必须满足：

1. `api.example.test` 的 DNS A/AAAA 记录指向 `198.51.100.10`；
